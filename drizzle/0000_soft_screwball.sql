...
CREATE TABLE `laboratoire` (
	...
	`origin` varchar(255),
	`extractionMethod` enum('distillation','extraction_solvant','co2_supercritique','co2_sous_critique','percolation_froide','expression','teinture','autre'),
	`olfactiveProfile` text,
	`character` text,
...
CREATE TABLE `pyrfume_datasets` (
	...
	`import_status` enum('pending','importing','completed','error') DEFAULT 'pending',
	`last_imported_at` timestamp,
	`created_at` timestamp NOT NULL DEFAULT (now()),
	CONSTRAINT `pyrfume_datasets_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `pyrfume_embeddings` (
	`id` int AUTO_INCREMENT NOT NULL,
	`molecule_id` int NOT NULL,
	`embedding_type` enum('morgan','mordred') NOT NULL,
	`embedding_vector` json NOT NULL,
	`dimensions` int,
	`imported_at` timestamp NOT NULL DEFAULT (now()),
	CONSTRAINT `pyrfume_embeddings_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `pyrfume_ifra_restrictions` (
	`id` int AUTO_INCREMENT NOT NULL,
	`molecule_id` int NOT NULL,
	...
	`source_url` varchar(500),
	`imported_at` timestamp NOT NULL DEFAULT (now()),
	CONSTRAINT `pyrfume_ifra_restrictions_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `pyrfume_molecule_mapping` (
	`id` int AUTO_INCREMENT NOT NULL,
	`molecule_id` int NOT NULL,
	`pyrfume_cid` int NOT NULL,
	`match_method` enum('cid','cas','smiles','name') NOT NULL,
	`confidence` float DEFAULT 1,
	`pyrfume_name` varchar(500),
	`pyrfume_smiles` text,
	`pyrfume_iupac` text,
	`pyrfume_mw` float,
	`matched_at` timestamp NOT NULL DEFAULT (now()),
	CONSTRAINT `pyrfume_molecule_mapping_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `pyrfume_olfactory_descriptors` (
	`id` int AUTO_INCREMENT NOT NULL,
	`molecule_id` int NOT NULL,
	...
	`source_url` varchar(500),
	`imported_at` timestamp NOT NULL DEFAULT (now()),
	CONSTRAINT `pyrfume_olfactory_descriptors_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
CREATE TABLE `variety_images` (
	...
	`cultivar` varchar(255),
	`imageType` enum('leaf','flower','fruit','bark','whole_plant','other') NOT NULL,
	`file_key` varchar(500) NOT NULL,
...
--> statement-breakpoint
CREATE TABLE `descriptor_molecule_links` (
	`id` int AUTO_INCREMENT NOT NULL,
	`descriptor_id` varchar(100) NOT NULL,
	`moleculeId` int NOT NULL,
	...
	CONSTRAINT `descriptor_molecule_links_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `descriptor_occurrences` (
	`id` int AUTO_INCREMENT NOT NULL,
	`descriptor_id` varchar(100) NOT NULL,
	`totalPlants` int DEFAULT 0,
	`totalMolecules` int DEFAULT 0,
	`lastUpdated` timestamp DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `descriptor_occurrences_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `descriptor_plant_links` (
	`id` int AUTO_INCREMENT NOT NULL,
	`descriptor_id` varchar(100) NOT NULL,
	...
	`createdAt` timestamp DEFAULT (now()),
	`updatedAt` timestamp DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `descriptor_plant_links_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
CREATE TABLE `plants` (
	...
	`itis_id` varchar(50),
	`pow_id` varchar(50),
	`ncbi_tax_id` varchar(20),
	`wikidata_qid` varchar(20),
	`wikidata_enriched_at` timestamp,
	`rdf_type` varchar(255),
	`dwc_taxon_rank` varchar(50),
	`notes` text,
	`image_url` varchar(500),
	`validation_status` enum('brouillon','en_revision','valide','rejete') DEFAULT 'valide',
	`validated_by` int,
...
ALTER TABLE `publication_extraction_methods` ADD CONSTRAINT `publication_extraction_methods_extraction_method_id_extraction_methods_id_fk` FOREIGN KEY (`extraction_method_id`) REFERENCES `extraction_methods`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX `bib_author_wikidata_idx` ON `bibliography_authors` (`wikidata_qid`);--> statement-breakpoint
CREATE INDEX `variety_genealogy_variety_idx` ON `variety_genealogy` (`variety_id`);--> statement-breakpoint
CREATE INDEX `variety_genealogy_parent_idx` ON `variety_genealogy` (`parent_variety_id`);--> statement-breakpoint
...
CREATE INDEX `pyrfume_datasets_name_idx` ON `pyrfume_datasets` (`name`);--> statement-breakpoint
CREATE INDEX `pyrfume_datasets_status_idx` ON `pyrfume_datasets` (`import_status`);--> statement-breakpoint
CREATE INDEX `pyrfume_embeddings_molecule_idx` ON `pyrfume_embeddings` (`molecule_id`);--> statement-breakpoint
CREATE INDEX `pyrfume_embeddings_type_idx` ON `pyrfume_embeddings` (`embedding_type`);--> statement-breakpoint
CREATE INDEX `pyrfume_embeddings_mol_type_idx` ON `pyrfume_embeddings` (`molecule_id`,`embedding_type`);--> statement-breakpoint
CREATE INDEX `pyrfume_ifra_molecule_idx` ON `pyrfume_ifra_restrictions` (`molecule_id`);--> statement-breakpoint
CREATE INDEX `pyrfume_ifra_restriction_idx` ON `pyrfume_ifra_restrictions` (`restriction_type`);--> statement-breakpoint
CREATE INDEX `pyrfume_ifra_category_idx` ON `pyrfume_ifra_restrictions` (`application_category`);--> statement-breakpoint
CREATE INDEX `pyrfume_mapping_molecule_idx` ON `pyrfume_molecule_mapping` (`molecule_id`);--> statement-breakpoint
CREATE INDEX `pyrfume_mapping_cid_idx` ON `pyrfume_molecule_mapping` (`pyrfume_cid`);--> statement-breakpoint
CREATE INDEX `pyrfume_mapping_unique_idx` ON `pyrfume_molecule_mapping` (`molecule_id`,`pyrfume_cid`);--> statement-breakpoint
CREATE INDEX `pyrfume_descriptors_molecule_idx` ON `pyrfume_olfactory_descriptors` (`molecule_id`);--> statement-breakpoint
CREATE INDEX `pyrfume_descriptors_dataset_idx` ON `pyrfume_olfactory_descriptors` (`dataset`);--> statement-breakpoint
CREATE INDEX `pyrfume_descriptors_descriptor_idx` ON `pyrfume_olfactory_descriptors` (`descriptor`);--> statement-breakpoint
CREATE INDEX `pyrfume_descriptors_mol_dataset_idx` ON `pyrfume_olfactory_descriptors` (`molecule_id`,`dataset`);--> statement-breakpoint
...
CREATE INDEX `variety_images_verified_idx` ON `variety_images` (`is_verified`);--> statement-breakpoint
CREATE INDEX `variety_images_sort_idx` ON `variety_images` (`genus`,`species`,`sort_order`);--> statement-breakpoint
...
CREATE INDEX `idx_descriptor_id` ON `descriptor_molecule_links` (`descriptor_id`);--> statement-breakpoint
CREATE INDEX `idx_molecule_id` ON `descriptor_molecule_links` (`moleculeId`);--> statement-breakpoint
CREATE INDEX `unique_descriptor_molecule` ON `descriptor_molecule_links` (`descriptor_id`,`moleculeId`);--> statement-breakpoint
CREATE INDEX `idx_descriptor_id` ON `descriptor_occurrences` (`descriptor_id`);--> statement-breakpoint
CREATE INDEX `idx_descriptor_id` ON `descriptor_plant_links` (`descriptor_id`);--> statement-breakpoint
CREATE INDEX `idx_plant_id` ON `descriptor_plant_links` (`plantId`);--> statement-breakpoint
CREATE INDEX `unique_descriptor_plant` ON `descriptor_plant_links` (`descriptor_id`,`plantId`);--> statement-breakpoint
...
CREATE INDEX `sparql_cache_created_idx` ON `sparql_cache` (`created_at`);--> statement-breakpoint
...
CREATE INDEX `bcc_target_id_idx` ON `bibliography_cross_citations` (`target_id`);--> statement-breakpoint
...
CREATE INDEX `idx_linked_molecule` ON `europeana_bookmarks` (`linked_molecule_id`);--> statement-breakpoint
