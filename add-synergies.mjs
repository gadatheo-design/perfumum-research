...
const molecules = await db.select({ id: schema.molecules.id, name: schema.molecules.name }).from(schema.molecules).limit(131);
const tabacs = await db.select({ id: schema.tabacs.id, name: schema.tabacs.name }).from(schema.tabacs);
...
for (const synergy of synergies) {
  await db.insert(schema.synergies).values({
    ...synergy,
    createdAt: new Date()
  });
