...
import { ChevronDown, ChevronUp, Search, ZoomIn, ZoomOut, RotateCcw, Maximize2, Minimize2 } from "lucide-react";
...
interface TreeNode {
  ...
  _collapsed?: boolean; // Internal state for collapse/expand
  _childCount?: number; // Cache for number of descendants
}

interface PhylogeneticTreeCollapsibleProps {
  data: {
    genus: string;
    species: string;
    rootNodes: TreeNode[];
...
function countDescendants(node: TreeNode): number {
  ...
  return node.children.reduce((sum, child) => 1 + sum + countDescendants(child), 0);
}

/**
 * Toggle collapse state of a node
 */
function toggleNodeCollapse(node: TreeNode): void {
  node._collapsed = !node._collapsed;
}

/**
 * Get visible children (respecting collapse state)
 */
function getVisibleChildren(node: TreeNode): TreeNode[] {
  if (node._collapsed || !node.children) return [];
 ...
 */
function filterNodes(nodes: TreeNode[], term: string): Set<number> {
 ...
 * Collapse all nodes
 */
function collapseAll(nodes: TreeNode[]): void {
  function traverse(node: TreeNode) {
...
export const PhylogeneticTreeCollapsible: React.FC<PhylogeneticTreeCollapsibleProps> = ({
  data,
  layout = "tree",
  onNodeSelect,
}) => {
  ...
  const [treeData, setTreeData] = useState<TreeNode[]>(data.rootNodes);
  ...
  useEffect(() => {
    if (searchTerm) {
      const matches = filterNodes(treeData, searchTerm);
  ...
  // Draw tree layout with collapse/expand
  const drawTreeLayout = useCallback(() => {
    if (!svgRef.current || !treeData.length) return;

    const width = containerRef.current?.clientWidth || 1200;
    const height = Math.max(600, treeData.length * 25 + 200);
    ...
    // Create hierarchy with visible children only
    const hierarchyData = {
      ...treeData[0],
  ...
  // Draw radial layout with collapse/expand
  const drawRadialLayout = useCallback(() => {
    if (!svgRef.current || !treeData.length) return;
  ...
  }, [layout, drawTreeLayout, drawRadialLayout]);
      ...
      <Card className="p-4">
        ...
        <div className="mt-3 pt-3 border-t border-gray-200">
          <p className="text-xs text-gray-600">
            💡 <strong>Tip:</strong> Click on nodes with children to collapse/expand branches. Yellow border = collapsed.
          </p>
        </div>
      </Card>
