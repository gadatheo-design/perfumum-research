...
export default function SourcingCannabis() {
  const [search, setSearch] = useState("");
  const [filterType, setFilterType] = useState<string>("all");

  const { data: suppliers, isLoading } = trpc.extendedSuppliers.getCannabisSuppliers.useQuery();
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-10 mt-6">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-zinc-100">Sourcing Cannabis</h1>
              <p className="text-zinc-400 text-sm">Fournisseurs de landraces, génétiques et extraits certifiés</p>
            </div>
          </div>
          <p className="text-zinc-400 max-w-3xl leading-relaxed">
            Répertoire des fournisseurs de cannabis pour la recherche olfactive et terpénique.
            Landraces himalayens, génétiques certifiées GMP, extraits standardisés — chaque source est
            documentée avec ses profils cannabinoïdes et terpéniques.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: "Fournisseurs", value: suppliers?.length || 0, icon: Building2, color: "text-emerald-400" },
          ...
          <div className="flex gap-2 flex-wrap">
            {supplierTypes.map(t => (
              <Button
                ...
                className={filterType === t.id ? "bg-emerald-700 hover:bg-emerald-800 text-white" : "border-zinc-700 text-zinc-400 hover:text-zinc-100"}
              >
                {t.label}
              </Button>
            ))}
        ...
        <div className="mb-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3">
          ...
          <p className="text-sm text-amber-300">
            <strong>Usage recherche uniquement.</strong> Ce répertoire est destiné exclusivement à la recherche olfactive et scientifique.
            Toute acquisition doit respecter la législation en vigueur dans votre pays.
          </p>
        </div>
                      ...
                      <div className="flex-1">
                        ...
                        {specialties.length > 0 && (
                          <div className="flex flex-wrap gap-1.5 mb-3">
                            {specialties.map(sp => (
                              <Badge key={sp} variant="outline" className="text-xs border-emerald-500/30 text-emerald-300 bg-emerald-500/10">
                                {sp}
                              </Badge>
                            ))}
                          </div>
          ...
          <Link href="/sourcing/tabac">
            <Card className="bg-zinc-900/50 border-zinc-800 hover:border-amber-500/30 transition-colors cursor-pointer">
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-medium text-zinc-200">Sourcing Tabac</div>
                  <div className="text-xs text-zinc-500">Virginia, Burley, Oriental</div>
                </div>
                <ChevronRight className="w-4 h-4 text-zinc-600 ml-auto" />
              </CardContent>
            </Card>
          </Link>
          <Link href="/cannabis-landraces">
            <Card className="bg-zinc-900/50 border-zinc-800 hover:border-emerald-500/30 transition-colors cursor-pointer">
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-medium text-zinc-200">Cannabis Landraces</div>
                  <div className="text-xs text-zinc-500">Génétiques & profils</div>
                </div>
                <ChevronRight className="w-4 h-4 text-zinc-600 ml-auto" />
              </CardContent>
            </Card>
          ...
          <Link href="/correlations">
            <Card className="bg-zinc-900/50 border-zinc-800 hover:border-purple-500/30 transition-colors cursor-pointer">
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-medium text-zinc-200">Corrélations Olfactives</div>
                  <div className="text-xs text-zinc-500">Tabac × Parfum × Cannabis</div>
                </div>
                <ChevronRight className="w-4 h-4 text-zinc-600 ml-auto" />
              </CardContent>
            </Card>
