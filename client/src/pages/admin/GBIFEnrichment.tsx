...
export default function GBIFEnrichment() {
  ...
  const searchMutation = trpc.gbifEnrichment.searchSpecies.useMutation();
  const batchMutation = trpc.gbifEnrichment.batchSearchSpecies.useMutation();
  const statsMutation = trpc.gbifEnrichment.getStats.useQuery();
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">🌍 GBIF Enrichment</h1>
          <p className="text-muted-foreground">Global Biodiversity Information Facility</p>
        </div>
        ...
        <Tabs defaultValue="search" className="w-full">
          ...
          <TabsContent value="batch" className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-4">
                <textarea
                  placeholder="Enter scientific names (one per line)..."
                  value={batchNames}
          ...
          <TabsContent value="stats" className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle>GBIF Statistics</CardTitle>
              </CardHeader>
              <CardContent>
                {statsMutation.data && (
                  <div className="space-y-2">
                    <p className="text-sm"><strong>Coverage:</strong> {statsMutation.data.notes?.coverage}</p>
                    <p className="text-sm"><strong>Occurrences:</strong> {statsMutation.data.notes?.occurrences}</p>
                  </div>
                )}
              </CardContent>
            </Card>
