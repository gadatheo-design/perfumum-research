...
interface LeafEconomyImageUploadProps {
  leafEconomyId: number;
...
export function LeafEconomyImageUpload({
  leafEconomyId,
  currentImageUrl,
  onImageUploaded,
  className,
  disabled = false,
}: LeafEconomyImageUploadProps) {
  ...
  const uploadMutation = trpc.leafEconomies.uploadImage.useMutation({
    onSuccess: (data) => {
  ...
  }, [leafEconomyId, uploadMutation, disabled]);
          ...
          >
            {previewUrl ? (
                ...
                {status === 'uploading' && (
                  <div className="absolute inset-0 bg-background/80 flex items-center justify-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm text-muted-foreground">Upload vers S3...</span>
                    </div>
                  </div>
                )}
                {status === 'success' && (
                  <div className="absolute inset-0 bg-emerald-500/20 flex items-center justify-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm text-emerald-600 font-medium">Image enregistrée</span>
                    </div>
                  </div>
                )}
              ...
              <div className="flex flex-col items-center gap-4 p-8 text-center">
                <div className={cn(
                  'p-4 rounded-full',
                  dragActive ? 'bg-emerald-100 dark:bg-emerald-900' : 'bg-muted'
                )}>
                  <ImageIcon className={cn(
                    'h-10 w-10',
                    dragActive ? 'text-emerald-600' : 'text-muted-foreground'
                  )} />
                </div>
                <div>
...
export default LeafEconomyImageUpload;
