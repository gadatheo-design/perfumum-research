...
describe("Session 06 Jan 2026 - Enrichissement avancé", () => {
  ...
  describe("Phase 3: Liaisons Tagetes lucida - Molécules", () => {
    it("should have Tagetes lucida (Pericón) in database", async () => {
      const plant = await db.getPlantById(300001);
      expect(plant).toBeDefined();
      expect(plant?.name).toBe("Pericón");
      expect(plant?.latinName).toBe("Tagetes lucida");
    });

    it("should have molecules linked to Tagetes lucida", async () => {
      const moleculeLinks = await db.getPlantMolecules(300001);
      ...
      // Vérifier les molécules attendues (structure: { molecule: {...}, percentageMin, ... })
    ...
    it("should have Méthyl-eugénol molecule in database", async () => {
      const molecule = await db.getMoleculeById(660003);
      expect(molecule).toBeDefined();
      expect(molecule?.name).toBe("Méthyl-eugénol");
      expect(molecule?.casNumber).toBe("93-15-2");
    });
  });
});
