...
export const appRouter = router({
  ...
  // ============================================================================
  // PHASE 2 - GENEALOGY ADVANCED (Généalogie avancée)
  // ============================================================================
  genealogyAdvanced: router({
    // Arbre généalogique complet
    getFullTree: publicProcedure
      .input(z.object({ 
        varietyId: z.number().int().min(1),
        maxDepth: z.number().int().min(1).max(20).default(10)
      }))
      .query(async ({ input }) => {
        return await db.getFullGenealogyTree(input.varietyId, input.maxDepth);
      }),
    ...
    getLineage: publicProcedure
      .input(z.object({ varietyId: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.getLineage(input.varietyId);
      }),
    
    // Généalogie avec profils moléculaires
    getWithMolecularProfiles: publicProcedure
      .input(z.object({ varietyId: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.getGenealogyWithMolecularProfiles(input.varietyId);
      }),
    ...
    getEvolutionTimeline: publicProcedure
      .input(z.object({ varietyId: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.getEvolutionTimeline(input.varietyId);
      }),
    
    // Variétés disparues liées
    findRelatedLostVarieties: publicProcedure
      .input(z.object({ varietyId: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.findRelatedLostVarieties(input.varietyId);
      }),
  }),

  // ============================================================================
  // LOST VARIETIES (Variétés disparues)
  // ============================================================================
  lostVarieties: router({
    list: publicProcedure.query(async () => {
      return await db.getAllLostVarieties();
    }),
    
    getById: publicProcedure
      .input(z.object({ id: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.getLostVarietyById(input.id);
      }),
    
    getByStatus: publicProcedure
      .input(z.object({ 
        status: z.enum(['extinct', 'extinct_in_wild', 'presumed_extinct', 'possibly_extinct', 'rediscovered'])
      }))
      .query(async ({ input }) => {
        return await db.getLostVarietiesByStatus(input.status);
      }),
    
    getByPlant: publicProcedure
      .input(z.object({ plantId: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.getLostVarietiesByPlant(input.plantId);
      }),
    
    search: publicProcedure
      .input(z.object({ query: z.string().min(1) }))
      .query(async ({ input }) => {
        return await db.searchLostVarieties(input.query);
      }),
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        latinName: z.string().optional(),
        plantId: z.number().optional(),
        extinctionStatus: z.enum(['extinct', 'extinct_in_wild', 'presumed_extinct', 'possibly_extinct', 'rediscovered']).default('presumed_extinct'),
        lastKnownDate: z.number().optional(),
        extinctionDate: z.number().optional(),
        extinctionCause: z.enum(['overexploitation', 'habitat_loss', 'climate_change', 'disease', 'hybridization', 'war_conflict', 'unknown']).optional(),
        extinctionDetails: z.string().optional(),
        historicalNames: z.array(z.string()).optional(),
        historicalRange: z.record(z.string(), z.any()).optional(),
        morphologicalDescription: z.string().optional(),
        olfactiveDescription: z.string().optional(),
        therapeuticUses: z.string().optional(),
        culturalSignificance: z.string().optional(),
        reconstructionPossibility: z.enum(['possible', 'partial', 'unlikely', 'impossible']).optional(),
        reconstructionNotes: z.string().optional(),
        closestLivingRelatives: z.array(z.number()).optional(),
        primarySources: z.array(z.string()).optional(),
        imageUrl: z.string().optional(),
      }))
      .mutation(async ({ input }) => {
        return await db.createLostVariety(input as any);
      }),
    ...
    update: protectedProcedure
      .input(z.object({
        ...
        name: z.string().optional(),
        latinName: z.string().optional(),
        extinctionStatus: z.enum(['extinct', 'extinct_in_wild', 'presumed_extinct', 'possibly_extinct', 'rediscovered']).optional(),
        extinctionCause: z.enum(['overexploitation', 'habitat_loss', 'climate_change', 'disease', 'hybridization', 'war_conflict', 'unknown']).optional(),
        extinctionDetails: z.string().optional(),
        olfactiveDescription: z.string().optional(),
        reconstructionPossibility: z.enum(['possible', 'partial', 'unlikely', 'impossible']).optional(),
      ...
      .mutation(async ({ input }) => {
        const { id, ...data } = input;
        return await db.updateLostVariety(id, data as any);
      }),
    ...
    bulkImport: protectedProcedure
      .input(z.array(z.object({
        ...
        therapeuticUses: z.string().optional(),
        culturalSignificance: z.string().optional(),
        hypotheticalMolecularProfile: z.record(z.string(), z.any()).optional(),
        reconstructionPossibility: z.enum(['possible', 'partial', 'unlikely', 'impossible']).optional(),
        reconstructionNotes: z.string().optional(),
        closestLivingRelatives: z.array(z.string()).optional(),
        primarySources: z.array(z.string()).optional(),
        imageUrl: z.string().optional(),
      })))
      .mutation(async ({ input }) => {
        return await db.bulkImportLostVarieties(input as any);
      }),
    
    // Statistiques
    getStats: publicProcedure.query(async () => {
      return await db.getLostVarietiesStats();
    }),
  }),

  // ============================================================================
  // MOLECULAR PROFILES (Profils moléculaires historiques)
  // ============================================================================
  molecularProfiles: router({
    list: publicProcedure.query(async () => {
      return await db.getAllMolecularProfiles();
    }),
    
    getById: publicProcedure
      .input(z.object({ id: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.getMolecularProfileById(input.id);
      }),
    
    getByVariety: publicProcedure
      .input(z.object({ varietyId: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.getMolecularProfilesByVariety(input.varietyId);
      }),
    
    getByPeriod: publicProcedure
      .input(z.object({ 
        period: z.enum(['prehistoric', 'ancient', 'medieval', 'renaissance', 'enlightenment', 'industrial', 'modern', 'contemporary'])
      }))
      .query(async ({ input }) => {
        return await db.getMolecularProfilesByPeriod(input.period);
      }),
    
    create: protectedProcedure
      .input(z.object({
        varietyId: z.number().int().min(1),
        historicalPeriod: z.enum(['prehistoric', 'ancient', 'medieval', 'renaissance', 'enlightenment', 'industrial', 'modern', 'contemporary']),
        yearEstimate: z.number().optional(),
        yearRangeStart: z.number().optional(),
        yearRangeEnd: z.number().optional(),
        sourceType: z.enum(['archaeological', 'historical_text', 'herbarium', 'genetic_analysis', 'reconstruction', 'contemporary_sample']),
        sourceDescription: z.string().optional(),
        sourceReferences: z.array(z.string()).optional(),
        molecularComposition: z.array(z.object({
          molecule: z.string(),
          percentage: z.number().optional(),
          range: z.string().optional(),
        })).optional(),
        terpeneProfile: z.record(z.string(), z.number()).optional(),
        olfactiveDescription: z.string().optional(),
        olfactiveNotes: z.record(z.string(), z.any()).optional(),
        confidenceLevel: z.enum(['confirmed', 'probable', 'hypothetical', 'speculative']).default('probable'),
        modernComparisonNotes: z.string().optional(),
        divergenceScore: z.number().min(0).max(100).optional(),
      }))
      .mutation(async ({ input }) => {
        return await db.createMolecularProfile(input as any);
      }),
  }),
  ...
  // ============================================================================
  molecularComparisons: router({
    list: publicProcedure.query(async () => {
      return await db.getAllMolecularComparisons();
    }),
    
    getById: publicProcedure
      .input(z.object({ id: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.getMolecularComparisonById(input.id);
      }),
    
    getByVariety: publicProcedure
      .input(z.object({ varietyId: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.getMolecularComparisonsByVariety(input.varietyId);
      }),
    
    create: protectedProcedure
      .input(z.object({
        ancientProfileId: z.number().int().min(1),
        modernVarietyId: z.number().int().min(1),
        lostVarietyId: z.number().optional(),
        overallSimilarity: z.number().min(0).max(100).optional(),
        terpeneProfileSimilarity: z.number().min(0).max(100).optional(),
        olfactiveProfileSimilarity: z.number().min(0).max(100).optional(),
        molecularDifferences: z.record(z.string(), z.any()).optional(),
        lostMolecules: z.array(z.string()).optional(),
        gainedMolecules: z.array(z.string()).optional(),
        analysisNotes: z.string().optional(),
        evolutionHypothesis: z.string().optional(),
        selectionPressures: z.array(z.string()).optional(),
        reconstructionRelevance: z.enum(['critical', 'important', 'useful', 'marginal']).default('useful'),
        reconstructionNotes: z.string().optional(),
        analyst: z.string().optional(),
      }))
      .mutation(async ({ input }) => {
        return await db.createMolecularComparison(input as any);
      }),
  }),

  // ============================================================================
  // HISTORICAL RECORDS (Archives historiques des variétés)
  // ============================================================================
  // ============================================================================
  // RESEARCH AXES NEZ (Axes de recherche du magazine NEZ)
  // ============================================================================
  researchAxesNez: router({
    list: publicProcedure.query(async () => {
      return await db.listResearchAxesNez();
    }),
    
    getBySlug: publicProcedure
      .input(z.object({ slug: z.string().min(1) }))
      .query(async ({ input }) => {
        return await db.getResearchAxisNezBySlug(input.slug);
      }),
    
    getById: publicProcedure
      .input(z.object({ axisId: z.string().min(1) }))
      .query(async ({ input }) => {
        return await db.getResearchAxisNezById(input.axisId);
      }),
    
    getWithSources: publicProcedure
      .input(z.object({ slug: z.string().min(1) }))
      .query(async ({ input }) => {
        return await db.getResearchAxisNezWithSources(input.slug);
      }),
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        novelty_tagline: z.string().optional(),
        description_fr: z.string().optional(),
        description_en: z.string().optional(),
        ui_module: z.string().optional(),
        core_entities: z.string().optional(),
        kpis: z.string().optional(),
        default_filters_json: z.record(z.string(), z.any()).optional(),
      }))
      .mutation(async ({ input }) => {
        return await db.createResearchAxisNez(input);
      }),
    ...
    bulkImport: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        return await db.bulkImportResearchAxesNez(input);
      }),
    
    getStats: publicProcedure.query(async () => {
      return await db.getResearchAxesNezStats();
    }),
  }),

  // ============================================================================
  // SOURCE ARTICLES NEZ (Articles sources du magazine NEZ)
  // ============================================================================
  sourceArticlesNez: router({
    list: publicProcedure.query(async () => {
      return await db.listSourceArticlesNez();
    }),
    
    getById: publicProcedure
      .input(z.object({ sourceId: z.string().min(1) }))
      .query(async ({ input }) => {
        return await db.getSourceArticleNezById(input.sourceId);
      }),
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        return await db.createSourceArticleNez(input as any);
      }),
    ...
    bulkImport: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        return await db.bulkImportSourceArticlesNez(input as any);
      }),
  }),

  // ============================================================================
  // AXIS SOURCE NEZ (Mappings axe-source NEZ)
  // ============================================================================
  axisSourcesNez: router({
    list: publicProcedure
      .input(z.object({ axisId: z.string().optional() }).optional())
      .query(async ({ input }) => {
        return await db.listAxisSourcesNez(input?.axisId);
      }),
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        return await db.createAxisSourceNez(input);
      }),
    ...
    bulkImport: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        return await db.bulkImportAxisSourcesNez(input);
      }),
  }),

  historicalRecords: router({
    list: publicProcedure.query(async () => {
      return await db.getAllHistoricalRecords();
    }),
    
    getById: publicProcedure
      .input(z.object({ id: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.getHistoricalRecordById(input.id);
      }),
    
    getByVariety: publicProcedure
      .input(z.object({ varietyId: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db.getHistoricalRecordsByVariety(input.varietyId);
      }),
    
    getByType: publicProcedure
      .input(z.object({ 
        recordType: z.enum(['botanical_description', 'trade_record', 'agricultural_manual', 'pharmacopoeia', 'herbarium_specimen', 'artistic_depiction', 'travel_account', 'scientific_paper', 'patent', 'oral_tradition', 'other'])
      }))
      .query(async ({ input }) => {
        return await db.getHistoricalRecordsByType(input.recordType);
      }),
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        mentionedCharacteristics: z.record(z.string(), z.any()).optional(),
        authenticityLevel: z.enum(['original', 'copy', 'transcription', 'reconstruction']).default('transcription'),
        reliabilityScore: z.number().min(1).max(10).optional(),
        sourceUrl: z.string().optional(),
      ...
      .mutation(async ({ input }) => {
        return await db.createHistoricalRecord(input as any);
      }),
  }),
  ...
  // ============================================================================
  researchAxesInnovants: router({
    list: publicProcedure.query(async () => {
      return await db.listResearchAxesInnovants();
    }),
    
    getByCode: publicProcedure
      .input(z.object({ axisCode: z.string().min(1) }))
      .query(async ({ input }) => {
        return await db.getResearchAxisInnovantByCode(input.axisCode);
      }),
    
    getByPriority: publicProcedure
      .input(z.object({ priority: z.enum(['critical', 'high', 'medium', 'low']) }))
      .query(async ({ input }) => {
        return await db.getResearchAxesInnovantsByPriority(input.priority);
      }),
    
    getByStatus: publicProcedure
      .input(z.object({ status: z.enum(['active', 'planned', 'completed', 'paused']) }))
      .query(async ({ input }) => {
        return await db.getResearchAxesInnovantsByStatus(input.status);
      }),
    
    create: protectedProcedure
      .input(z.object({
        axis_code: z.string().min(1),
        title_fr: z.string().min(1),
        ...
        kpis: z.record(z.string(), z.number()).optional(),
        priority_level: z.enum(['critical', 'high', 'medium', 'low']).default('medium'),
        status: z.enum(['active', 'planned', 'completed', 'paused']).default('planned'),
      }))
      .mutation(async ({ input }) => {
        return await db.createResearchAxisInnovant(input);
      }),
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        return await db.updateResearchAxisInnovant(input.axisCode, input.data);
      }),
